...
sub build_service {
	...
	try {
		...
		$service = $factory->build_service($service_meta->implements,$self->container);
	} catch {
		ServiceCreationException->throw(message=>"The factory ".$service_meta->class_name." could not be created: $_");
	};
